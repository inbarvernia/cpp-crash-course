...
int main() {
  ...
  // You can also define the size of an array by using a variable
  int a;
  std::cin >> a;

  int variableArray[a] = {10, 24, 36};

  for(int i = 0; i < a; i++) {
    std::cout << variableArray[i] << std::endl;
  }
  // This isn't supported in all C++ versions; in some versions you have to use a constant rather than a variable, and in some versions you have to use a special function/keyword malloc if you want dynamically sized arrays
